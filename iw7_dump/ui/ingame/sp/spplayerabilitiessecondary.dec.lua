...
f0_local0 = function ( f1_arg0 )
	if Engine.IsGamepadEnabled() == 1 then
		ACTIONS.AnimateSequence( f1_arg0, "SecondaryAbilityAvailable" )
	else
		ACTIONS.AnimateSequence( f1_arg0, "SecondaryAbilityAvailablePC" )
	end
end

f0_local1 = function ( f2_arg0 )
	if Engine.IsGamepadEnabled() == 1 then
		ACTIONS.AnimateSequence( f2_arg0, "SecondaryAbilityUnavailable" )
	else
		ACTIONS.AnimateSequence( f2_arg0, "SecondaryAbilityUnavailablePC" )
	end
end
...
function PostLoadFunc( f3_arg0, f3_arg1, f3_arg2 )
	...
	if f3_local1 then
		...
		if f3_local1 then
			f3_local1 = f3_arg0.ButtonLB
			if f3_local1 then
				f3_local1 = f3_arg0.Frame
				if f3_local1 then
	...
	f3_local0( f3_local1, "WARNING: SP Player Abilities Secondary Items Are Missing..." )
	...
	f3_local0 = DataSources.inGame.player.secondaryPower.rechargeProgress
	f3_local1 = DataSources.inGame.player.secondaryPower.type
	local f3_local2 = DataSources.inGame.player.secondaryPower.powerDisabled
	local f3_local3 = DataSources.inGame.player.secondaryPower.reference
	local f3_local4 = DataSources.inGame.player.secondaryPower.lowPower
	local f3_local5 = DataSources.inGame.player.secondaryPower.numCharges
	local f3_local6 = DataSources.inGame.player.secondaryStoredPower.name
	...
	local f3_local8 = function ()
		local f5_local0 = DataSources.alwaysLoaded.input.commands.smokeIcon:GetValue( f3_arg1 )
		if Engine.IsConsoleGame() then
			if f5_local0 ~= nil then
				if f5_local0 == "button_primary" then
					f3_arg0.ButtonLB:SetAnchorsAndPosition( 0.5, 0.5, 1, 0, _1080p * -12.5, _1080p * 12.5, _1080p * -30, _1080p * -5 )
				else
					f3_arg0.ButtonLB:SetAnchorsAndPosition( 0.5, 0.5, 1, 0, _1080p * -20, _1080p * 20, _1080p * -30, _1080p * -5 )
				end
				f3_arg0.ButtonLB:setImage( RegisterMaterial( f5_local0 ), 0 )
			end
	...
	local f3_local9 = function ()
		if Engine.IsGamepadEnabled() == 1 then
			...
			local f6_local0 = Engine.GetKeyBindingLocalizedString( f3_arg1, "+smoke", true )
			if string.len( f6_local0 ) < 6 then
				f3_arg0.KeyLabel:SetAnchorsAndPosition( 0, 0, 1, 0, _1080p * 0, 0, _1080p * -25, _1080p * -9 )
				f3_arg0.KeyLabel:SetAlignment( LUI.Alignment.Center )
			else
				f3_arg0.KeyLabel:SetAnchorsAndPosition( 0, 0, 1, 0, _1080p * -30, 0, _1080p * -25, _1080p * -9 )
				f3_arg0.KeyLabel:SetAlignment( LUI.Alignment.Right )
			end
	...
	f3_arg0:SubscribeToModel( DataSources.alwaysLoaded.input.commands.smokeIcon:GetModel( f3_arg1 ), f3_local8 )
	...
	f3_arg0:SubscribeToModel( f3_local0:GetModel( f3_arg1 ), function ()
		local f10_local0 = f3_local1:GetValue( f3_arg1 )
		local f10_local1 = f3_local3:GetValue( f3_arg1 )
		local f10_local2 = f3_local2:GetValue( f3_arg1 )
		local f10_local3 = f3_local4:GetValue( f3_arg1 ) * 1000
		local f10_local4 = f3_local0:GetValue( f3_arg1 )
		local f10_local5 = f10_local4 / 1000
		if f10_local0 == "drain" then
			f3_arg0.Frame.Background:SetTopAnchor( 1 - f10_local5 )
			local f10_local6 = f3_arg0.Frame.Background
			local f10_local7 = f10_local6
			f10_local6 = f10_local6.SetRGBFromTable
			local f10_local8
			if f10_local5 < 1 then
				f10_local8 = SWATCHES.powers.recharging_sp
			...
			f10_local8 = SWATCHES.powers.background
		else
			f3_arg0.Frame.Background:SetTopAnchor( 0 )
			f3_arg0.Frame.Background:SetRGBFromTable( SWATCHES.powers.background )
		end
		if f10_local4 >= 1000 and f10_local0 == "drain" then
			ACTIONS.AnimateSequenceByElement( f3_arg0, {
				elementName = "SecondaryAbility",
				sequenceName = "Available",
				elementPath = ""
		...
		if f10_local4 < 1000 and f10_local4 > 0 and f10_local0 == "drain" then
			ACTIONS.AnimateSequenceByElement( f3_arg0, {
				elementName = "SecondaryAbility",
			...
			if 1 == Engine.IsGamepadEnabled() then
				f3_arg0.ButtonLB:SetAlpha( 1, 0 )
			else
				f3_arg0.KeyLabel:SetAlpha( 1, 0 )
			end
		end
		if not (f10_local4 ~= 0 or f10_local0 ~= "drain" or not f10_local1 ~= "none") or f10_local2 then
			ACTIONS.AnimateSequenceByElement( f3_arg0, {
				elementName = "SecondaryAbility",
				sequenceName = "Unavailable",
				elementPath = ""
		...
		if f10_local4 < f10_local3 and not f10_local2 and f10_local0 == "drain" then
...
function SPPlayerAbilitiesSecondary( menu, controller )
	local self = LUI.UIElement.new()
	self:SetAnchorsAndPosition( 0, 1, 0, 1, 0, 70 * _1080p, 0, 115 * _1080p )
	self.id = "SPPlayerAbilitiesSecondary"
	self._animationSets = {}
	self._sequences = {}
	...
	Arrow:SetAnchorsAndPosition( 0, 1, 0.5, 0.5, _1080p * -30, _1080p * -10, 0, _1080p * 20 )
	...
	ButtonLB = LUI.UIImage.new()
	ButtonLB.id = "ButtonLB"
	ButtonLB:SetRGBFromTable( SWATCHES.HUD.primary, 0 )
	ButtonLB:SetScale( -0.2, 0 )
	ButtonLB:setImage( RegisterMaterial( "hud_but_blank_l" ), 0 )
	ButtonLB:SetAnchorsAndPosition( 0.5, 0.5, 1, 0, _1080p * -20, _1080p * 20, _1080p * -30, _1080p * -5 )
	self:addElement( ButtonLB )
	...
	SecondaryAbility:SetAlpha( 0.75, 0 )
	SecondaryAbility:SetScale( -0.25, 0 )
	SecondaryAbility:SetDataSource( DataSources.inGame.player.secondaryPower, f12_local1 )
	SecondaryAbility:SetAnchorsAndPosition( 0.5, 0.5, 0, 1, _1080p * -40, _1080p * 40, _1080p * 11, _1080p * 91 )
	...
	Quantity:SetShadowMinDistance( -0.02, 0 )
	Quantity:SetShadowMaxDistance( 0.02, 0 )
	Quantity:SetAnchorsAndPosition( 0, 0, 0, 1, 0, 0, _1080p * 1, _1080p * 17 )
	Quantity:BindColorToModel( DataSources.inGame.player.secondaryPower.color:GetModel( f12_local1 ) )
	Quantity:BindAlphaToModel( DataSources.inGame.player.secondaryPower.numAlpha:GetModel( f12_local1 ) )
	...
	self._animationSets.DefaultAnimationSet = function ()
		...
		ButtonLB:RegisterAnimationSequence( "SecondaryAbilityAvailable", {
			{
				function ()
					return self.ButtonLB:SetAlpha( 1, 0 )
				end
			},
			{
				function ()
					return self.ButtonLB:SetScale( 0.25, 0 )
				end,
				function ()
					return self.ButtonLB:SetScale( -0.2, 100 )
				end
			}
		} )
		self._sequences.SecondaryAbilityAvailable = function ()
			ButtonLB:AnimateSequence( "SecondaryAbilityAvailable" )
		end
		
		ButtonLB:RegisterAnimationSequence( "SecondaryAbilityUnavailable", {
			{
				function ()
					return self.ButtonLB:SetAlpha( 0.2, 0 )
				end
			}
		} )
		self._sequences.SecondaryAbilityUnavailable = function ()
			ButtonLB:AnimateSequence( "SecondaryAbilityUnavailable" )
		end
		...
		KeyLabel:RegisterAnimationSequence( "SecondaryAbilityAvailablePC", {
			{
				function ()
		...
		self._sequences.SecondaryAbilityAvailablePC = function ()
			KeyLabel:AnimateSequence( "SecondaryAbilityAvailablePC" )
		end
		
		KeyLabel:RegisterAnimationSequence( "SecondaryAbilityUnavailablePC", {
			{
				function ()
		...
		self._sequences.SecondaryAbilityUnavailablePC = function ()
			KeyLabel:AnimateSequence( "SecondaryAbilityUnavailablePC" )
		end
...
MenuBuilder.registerType( "SPPlayerAbilitiesSecondary", SPPlayerAbilitiesSecondary )
LockTable( _M )
