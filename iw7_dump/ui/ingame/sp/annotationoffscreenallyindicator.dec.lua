...
function AnnotationOffscreenAllyIndicator( menu, controller )
	local self = LUI.UIElement.new()
	self:SetAnchorsAndPosition( 0, 1, 0, 1, 0, 32 * _1080p, 0, 32 * _1080p )
	self.id = "AnnotationOffscreenAllyIndicator"
	self._animationSets = {}
	self._sequences = {}
	...
	Indicator = MenuBuilder.BuildRegisteredType( "JackalOffscreenIndicatorPoint", {
		controllerIndex = f1_local1
	} )
	Indicator.id = "Indicator"
	Indicator:SetAlpha( 0, 0 )
	Indicator:SetScale( -0.5, 0 )
	Indicator.ArrowLg:SetRGBFromTable( SWATCHES.jackal.notification, 0 )
	...
	self._animationSets.DefaultAnimationSet = function ()
		...
		Indicator:RegisterAnimationSequence( "fadeIn", {
			{
				function ()
		...
		self._sequences.fadeIn = function ()
			Indicator:AnimateSequence( "fadeIn" )
		end
		
		Indicator:RegisterAnimationSequence( "fadeOut", {
			{
				function ()
		...
		self._sequences.fadeOut = function ()
			Indicator:AnimateSequence( "fadeOut" )
		end
...
MenuBuilder.registerType( "AnnotationOffscreenAllyIndicator", AnnotationOffscreenAllyIndicator )
LockTable( _M )
