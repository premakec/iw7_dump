...
f0_local0 = function ( f1_arg0, f1_arg1, f1_arg2 )
	f1_arg0:registerEventHandler( "empty_menu_stack", function ( element, event )
		element.hint:setText( Engine.Localize( "COOP_INTERACTIONS_BFP_HINT" ), 0 )
	end )
end

function bfpHelper( menu, controller )
	...
	bg = LUI.UIImage.new()
	bg.id = "bg"
	bg:SetRGBFromTable( SWATCHES.Reticles.AzureBlue, 0 )
	bg:setImage( RegisterMaterial( "cp_zmb_game_helper" ), 0 )
	bg:SetAnchorsAndPosition( 0, 1, 0, 1, _1080p * 1218.7, _1080p * 1730, _1080p * 739, _1080p * 995 )
	self:addElement( bg )
	self.bg = bg
	...
	title:setText( "Bowling for Planets", 0 )
	...
	hint = LUI.UIText.new()
	hint.id = "hint"
	hint:SetRGBFromTable( SWATCHES.AAR.featuredRankProgress, 0 )
	hint:setText( Engine.Localize( "COOP_INTERACTIONS_BFP_HINT" ), 0 )
	hint:SetFontSize( 16 * _1080p )
	hint:SetFont( FONTS.GetFont( FONTS.ZmClean.File ) )
	hint:SetAlignment( LUI.Alignment.Center )
	hint:SetAnchorsAndPosition( 0, 1, 0, 1, _1080p * 1320, _1080p * 1629, _1080p * 906, _1080p * 922 )
	...
	topper.id = "topper"
	topper:setImage( RegisterMaterial( "cp_zmb_bfp_top" ), 0 )
	...
	self:SubscribeToModel( DataSources.inGame.CP.zombies.bfpHelper:GetModel( f3_local1 ), function ()
		if DataSources.inGame.CP.zombies.bfpHelper:GetValue( f3_local1 ) ~= nil and DataSources.inGame.CP.zombies.bfpHelper:GetValue( f3_local1 ) == 1 then
			ACTIONS.AnimateSequence( self, "show" )
		end
		if DataSources.inGame.CP.zombies.bfpHelper:GetValue( f3_local1 ) ~= nil and DataSources.inGame.CP.zombies.bfpHelper:GetValue( f3_local1 ) == 0 then
...
MenuBuilder.registerType( "bfpHelper", bfpHelper )
LockTable( _M )
